...
export class IdentityServerDebugSession extends LoggingDebugSession {
	...
	private _runtime: RemoteIdentityServerRuntime;
	...
	public constructor() {
		...
		this._runtime = new RemoteIdentityServerRuntime();
		...
		this._runtime.on('breakpointValidated', (bp: RemoteBreakpoint) => {
		...
		rpc.listen({
			...
			onConnection: (rpcConnection: rpc.MessageConnection) => {
				...
				rpcConnection.onNotification(notification, (param: any) => {
					...
					this._runtime.fireBreakpoint(param.line);
				});
